...
export default {
    ...
    methods: {
        pullList() {
            ...
            this.loading = true
            const { url, data = {} } = this._getPullParams()
            this.$http.get(url, data, ({ data }) => {
                if (data.length > 0) {
                ...
                this.page++
                this.loading = this.refreshing = false // 请求完成
            })
        },
        refreshList() { // swipe down to refresh
